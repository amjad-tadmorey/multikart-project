...
const DesktopSecondaryNav = () => {
    return (
        <nav className="hidden special:block">
            ...
            {/* PAGES */}
            <div className="group py-3 relative">
                <a href="/" className="hover:text-brand transition-colors duration-500 flex justify-between items-center gap-1">
                    Pages <RiArrowDownSLine className='rotate-270' size={22} />
                </a>
                ...
                <div className="absolute top-0 left-70 pt-4 opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-500 ease-in-out z-50 pointer-events-none group-hover:pointer-events-auto">
                    ...
                    <div className="w-64 py-4 px-4 overflow-x-visible relative">
                        <div className="flex flex-col text-lg font-normal text-gray-500">
                            <div className="flex flex-col text-lg font-normal text-gray-500 w-64 bg-white p-2 rounded-md shadow-md">
                                {SECONDARY_NAV.pages.map((l1) => (
                                    /* LEVEL 1 WRAPPER */
                                                        ...
                                                        if (l2.subLinks) {
                                                            return (
                                                                <div key={l2.title} className="relative group/level3 w-full">
                                                                    ...
                                                                    <div className="flex items-center justify-between px-2 py-1.5 text-lg text-gray-600 hover:bg-gray-50 hover:text-brand rounded transition-colors duration-300 cursor-pointer">
                                                                        ...
                                                                        <div className="w-52 bg-white border border-gray-100 shadow-xl rounded-md p-3 flex flex-col gap-1">
                                                                            {l2.subLinks.map((l3) => (
                                                                                <a
                                                                                    key={l3.title}
                                                                                    href={l3.path}
                                                                                    className="px-2 py-1.5 text-lg text-gray-600 hover:bg-gray-50 hover:text-brand rounded transition-colors duration-300"
                                                                                >
                                                                                    {l3.title}
                                                                                </a>
                                                                            ))}
